...
public abstract class AbstractPart extends Entity {

    private static final TrackedData<Optional<UUID>> PARENT_UUID = DataTracker.registerData(AbstractPart.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
    private static final TrackedData<Float> SCALE_WIDTH = DataTracker.registerData(AbstractPart.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> SCALE_HEIGHT = DataTracker.registerData(AbstractPart.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> PART_YAW = DataTracker.registerData(AbstractPart.class, TrackedDataHandlerRegistry.FLOAT);
    ...
    protected AbstractPart(EntityType<?> t, World world) {
        super(t, world);
        this.multipartSize = t.getDimensions();
    }

    protected AbstractPart(EntityType<?> t, Entity parent, float radius, float angleYaw, float offsetY, float sizeX, float sizeY, float damageMultiplier) {
    ...
    public void collideWithNearbyEntities() {
        ...
        Entity parent = this.getParent();
        if (parent != null) {
            entities.stream().filter(entity -> entity != parent && !(entity instanceof AbstractPart) && entity.isPushable()).forEach(entity -> entity.pushAwayFrom(parent));
        }
    }
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        if (this.getWorld().isClient && this.getParentId() != null) {
            ...
            ClientPlayNetworking.send(new Identifier(ResonantMain.MOD_ID, "multipart_interact"), buf);
        }
    ...
    public boolean isInvulnerableTo(DamageSource source) {
        return source.isFromFalling() || source.isFallingBlock() || isInLava() || isOnFire() || super.isInvulnerableTo(source);
    }
