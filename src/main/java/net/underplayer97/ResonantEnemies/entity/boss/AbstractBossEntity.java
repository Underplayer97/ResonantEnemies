...
public abstract class AbstractBossEntity extends HostileEntity {
    ...
    public void pushEntity(LivingEntity entity, float power) {
        ...
        entity.setVelocity((entity.getX() - this.getX()) * power + vec.getX(), 0.2D,
                (entity.getZ() - this.getZ() * power + vec.getZ()));
    ...
    public void tickMovement() {
        ...
        if (this.getInvulnerableTimer() > 0) {
            for (int d = 0; d < 3; ++d){
                this.getWorld().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() +
                        this.random.nextGaussian(), this.getY() + (double)(this.random.nextFloat() * 3.3F),
                        this.getZ() + this.random.nextGaussian(),  0.699999988079071D,
